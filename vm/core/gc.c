...
visit_field1:
				...
				IF_GC_TRACE(fprintf (stderr, "case 7\n"));
			} else {
				IF_GC_TRACE(fprintf (stderr, "case 8\n"));
			}
...
pop:
		IF_GC_TRACE(fprintf (stderr, "pop    stack=%d  visit=%d (tag=%d)\n", stack, visit, ram_get_gc_tags (visit)>>6));

		if (stack != OBJ_FALSE) {
			if (HAS_2_OBJECT_FIELDS(stack) && ram_get_gc_tag1 (stack)) {
				IF_GC_TRACE(fprintf (stderr, "case 9\n"));
			...
			IF_GC_TRACE(fprintf (stderr, "case 11\n"));
...
void gc ()
{
	...
	IF_TRACE(fprintf(stderr, "\nGC BEGINS\n"));

	IF_GC_TRACE(fprintf(stderr, "arg1\n"));
	mark (arg1);
	IF_GC_TRACE(fprintf(stderr, "arg2\n"));
	mark (arg2);
	IF_GC_TRACE(fprintf(stderr, "arg3\n"));
	mark (arg3);
	IF_GC_TRACE(fprintf(stderr, "arg4\n"));
	mark (arg4);
	IF_GC_TRACE(fprintf(stderr, "cont\n"));
	mark (cont);
	IF_GC_TRACE(fprintf(stderr, "env\n"));
	mark (env);
	...
	IF_GC_TRACE(fprintf(stderr, "globals\n"));
